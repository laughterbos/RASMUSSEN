...
        <div data-role="main" class="ui-content">            <?php                        $servername = "us-cdbr-azure-southcentral-f.cloudapp.net";            $username = "b13fe870529434";            $password = "b3092936";            $dbname = "acsm_9ab42719d04fd9b";            $UserID = $_POST['User ID'];            // Create connection            $conn = mysqli_connect($servername, $username, $password, $dbname);            // Check connection            if (!$conn) {                die("Connection failed: " . mysqli_connect_error());            }            $sql= "SELECT Notes.Text, additionalData.Value FROM notes left join additionalData on Notes.User = additionalData.id WHERE Notes.User = 1";            $result = mysqli_query($conn, $sql);            if (mysqli_num_rows($result) > 0) {            // output data of each row            while($row = mysqli_fetch_assoc($result)) {                echo "<b>Note: " . $row['Text']. "</b><br>";                echo "<b>Additional Data: " . $row['Value']. "</b><br>";            }            } else {                echo "Sorry there are no matches! Please check your entry and try again.";            }            mysqli_close($conn);            ?>
